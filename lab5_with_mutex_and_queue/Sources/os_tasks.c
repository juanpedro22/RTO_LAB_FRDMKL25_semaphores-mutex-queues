 ...
 *
 * Behavior change to avoid orange:
 *  - Task1 sets RED, waits 500 ms, then clears RED,
 *    and only AFTER clearing it it puts a MSG_BLINK_GREEN into the queue.
 *  - Task2 waits on the queue and performs a GREEN blink cycle when message arrives.
 *
 * This guarantees GREEN is set only after RED has been cleared => no overlap.
 */
        ...
        switch (currentState) {
            case STATE_RED_ON:
                ...
                /* Only after red has been cleared, notify Task2 via msg queue */
                {
                    ...
                    /* In production you should handle failures (e.g. queue full).
                       For lab/demo we simply ignore or spin if needed. */
                    (void)st;
                }

                /* Transition to RED_OFF state and perform OFF delay */
                currentState = STATE_RED_OFF;
                break;

            case STATE_RED_OFF:
                /* OFF period: nothing to do, wait 500 ms */
                vTaskDelay(pdMS_TO_TICKS(500));
...
void Task2_task(os_task_param_t task_init_data)
{
    ...
    while (1) {
        ...
        switch (currentState) {
            ...
            case STATE_WAIT_MSG:
                /* Block indefinitely waiting for a message */
                ...
                if (st == kStatus_OSA_Success) {
                    if (received_msg == MSG_BLINK_GREEN) {
                        ...
                        /* Unknown message: ignore and continue waiting */
                        currentState = STATE_WAIT_MSG;
                    }
                } else {
                    /* On error, continue waiting (robustness) */
                    currentState = STATE_WAIT_MSG;
                }
                break;
            ...
            case STATE_GREEN_OFF:
                ...
                vTaskDelay(pdMS_TO_TICKS(300));
                /* After completing blink cycle, return to waiting for next message */
                currentState = STATE_WAIT_MSG;
                break;
